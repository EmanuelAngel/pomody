...
- **Descripción:** Sistema de catálogo de actividades de descanso (con sugerencias predefinidas y personalización del usuario) que se auto-asignan automáticamente mediante un botón _"Revitalización automática"_ o se seleccionan manualmente al iniciar un break. Sugiere actividades saludables (caminar, hidratarse, estirar, mirar a lo lejos) en lugar de dispersión pasiva (scroll o redes).
- **Justificación (Qué valor aporta):** Elimina la fricción y la duda al llegar al descanso (_"¿qué hago ahora?"_), promoviendo pausas efectivas que realmente recarguen energía para el siguiente bloque.
- **¿Depende de otra funcionalidad?:** App Base (v0.1).
- **Complejidad estimada:** Baja.
...
- **Descripción:** Integración opcional para comunicar el estado de concentración y blindar al usuario contra interrupciones:
  1. _Discord Rich Presence_: Muestra en el perfil de Discord el bloque activo, tiempo restante y la tarea en curso (con opción de privacidad para mostrar solo "Modo Foco").
  2. _Modo No Molestar del SO_: Activa automáticamente el modo de concentración o "No molestar" del sistema operativo mientras corre un bloque de focus para silenciar notificaciones.

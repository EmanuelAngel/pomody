...
## 8. Texto Estático Hardcodeado en Botón de Restablecer Valores Predeterminados

- **ID Referencia:** JD-17
- **Ubicación:** `src/lib/components/settings/settings-drawer.svelte`
- **Descripción:** El botón de restablecer valores (`handleResetDefaults`) presenta el texto estático `Reset to defaults (25 / 5 / 15 min · 4 rounds)` hardcodeado en la plantilla en lugar de interpolar dinámicamente las duraciones y rondas desde `DEFAULT_TIMER_CONFIG` (ej. calculando los minutos mediante `DEFAULT_TIMER_CONFIG.focusDurationSeconds / 60`, etc.). Esto genera acoplamiento rígido en la UI e incongruencias visuales ante cualquier ajuste de la configuración base.
